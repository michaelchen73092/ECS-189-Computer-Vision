...
[row,col]=find(accumulatorarray(:,:,radius)>thresholdvalue*max(max(max(accumulatorarray))) & accumulatorarray(:,:,radius)<0.9*max(max(max(accumulatorarray))) );  %%find voting result that larger 0.9*max
...
title('jupiter.jpg with multi-radius');    
end %function end